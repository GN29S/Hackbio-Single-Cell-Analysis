...
def dna_to_protein(dna_sequence):
    # Convert input to uppercase to handle 'a', 't', 'c', 'g'
    ...
    if len(dna_sequence) % 3 != 0:
        raise ValueError("Invalid DNA sequence length: must be a multiple of 3")
    ...
    for i in range(0, len(dna_sequence) - 2, 3):  # Read in triplets
        ...
        if amino_acid is None: # Handle codons not in the table, though the current table is exhaustive for valid DNA codons
            raise ValueError(f"Unknown codon: {codon} found in sequence")
...
def hamming_distance(str1, str2):
  ...
  with spaces to match the length of the longer string before comparison.
...
#usernames
slack_username = " team glutamic acid"
twitter_handle = "@team_glutamic_acid"

#calculate the hamming distance of team glutamic acid
